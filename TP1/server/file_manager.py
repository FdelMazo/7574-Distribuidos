...
class FileManager:
    ...
    The way of handling concurrency is throughout locking each file, with our own RWLock
    (multiple readers, one writer).

    For this we'll dynamically create one lock per file, the first time we need to
    access them.
    ...
    def __init__(self, process_manager_lock):
        """As our way of handling concurrency is through the multiprocessing module, we
        need our global lock to be a multiprocessing.Manager().Lock() object. In any
        other case, a simple threading.lock() would suffice."""
    ...
    def get_lines(self, filename, start_time, end_time):
        ...
        Both start_time and end_time can be None, in which case we simply don't filter
        by them.
        ...
        For this to work, this method (heavily) assumes that each line
          ...
          followed by a space, and that the lines are in ascending order. (this
          assumptions are typically true for log files...)

        This method is optimized for getting the last lines of a file (i.e, our
        start_time is 'young')
          (we want our most recent logs!).

          - Instead of reading the whole file and fitting it into memory,
              we'll read it line by line starting from the bottom until we reach the
              start_time and then we'll filter by the end_time. So, if the end_time is
              too 'old', we are 'over-reading' the file."""
        lines = []
        self.get_lock(filename).acquire_read()
